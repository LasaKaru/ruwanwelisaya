...
/* donate.jsx — donation flow with Stripe / PayPal / Bank transfer */
...
function StripeMark() {
  return (
    <svg viewBox="0 0 60 25" width="48" height="18" aria-label="Stripe">
      <text x="0" y="20" fontFamily="Inter, sans-serif" fontWeight="700" fontStyle="italic" fontSize="20" fill="#635bff">stripe</text>
    </svg>
  );
}
...
function CardForm() {
  return (
    <div className="rw-pay-form">
      ...
      <p className="rw-pay-form__note">
        Payments processed by <StripeMark/>. Card details never touch our servers.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="rw-pay-form rw-pay-form--paypal">
      <div className="rw-paypal-box">
        <div className="rw-paypal-box__mark"><PaypalMark/></div>
  ...
  return (
    <div className="rw-pay-form rw-pay-form--bank">
      ...
      <dl className="rw-bank">
        ...
        <div><dt>Reference</dt><dd className="rw-mono">DONATE-{Math.random().toString(36).slice(2,8).toUpperCase()}</dd></div>
      </dl>
...
function DonatePage() {
  ...
  const methods = [
    flags.paymentStripe && { id: "card",    label: "Card",    sub: "Visa, Mastercard, Amex", mark: <StripeMark/> },
    flags.paymentPaypal && { id: "paypal",  label: "PayPal",  sub: "Redirect to PayPal",     mark: <PaypalMark/> },
    flags.paymentBank   && { id: "bank",    label: "Bank",    sub: "LKR transfer",           mark: <Icon name="shield" size={18} color="#d4af37"/> },
  ].filter(Boolean);

  // If currently-selected method got disabled, fall back
  React.useEffect(() => {
    if (!methods.find((m) => m.id === method) && methods[0]) setMethod(methods[0].id);
  }, [flags]);
      ...
      <header className="rw-page__header">
        <div className="rw-container">
          ...
          <p className="rw-page__lead">
            Your contribution helps preserve one of humanity's most enduring monuments of faith. Every rupee, no matter the size, supports the ongoing care of the Great Stupa.
          </p>
        </div>
      </header>
          ...
          <div className="rw-donate-card">
            {submitted ? (
              <div className="rw-donate__success">
                ...
                <Button variant="secondary" onClick={() => setSubmitted(false)}>Make Another</Button>
              </div>
            ) : (
              <form className="rw-donate__form" onSubmit={submit}>
                ...
                <div className="rw-donate__group">
                  ...
                  <div className="rw-amount-grid">
                    ...
                    <div className={`rw-amount rw-amount--custom ${custom ? "is-active" : ""}`}>
                      <span>Rs.</span>
                      <input type="number" inputMode="numeric" placeholder="Other"
                  ...
                  <div className="rw-pay-methods">
                    {methods.map((m) => (
                      <button type="button" key={m.id}
                        className={`rw-pay-method ${method === m.id ? "is-active" : ""}`}
                ...
                {method !== "bank" && (
                  <>
                    ...
                    <Button type="submit" variant="primary" size="lg" className={`rw-donate__submit ${pulse ? "is-pulsing" : ""}`}>
                ...
                <p className="rw-donate__legal">
                  Payments processed securely. Ruwanwelisaya Preservation is a registered non-profit. Receipts are tax-deductible in Sri Lanka.
                </p>
              </form>
            )}
          </div>
          ...
          <div className="rw-donate__aside">
            ...
            <FadeIn delay={0.15}>
              <div className="rw-donate__quote">
                ...
                <blockquote>
                  &ldquo;If you light a lamp for somebody, it will also brighten your path.&rdquo;
                </blockquote>
