...
const FORUM_POSTS = [
  {
    avatar: 'AS', name: 'Anura Senanayake', where: 'Kandy, Sri Lanka', when: '2 hours ago',
  ...
  {
    avatar: 'PK', name: 'Priya Kumarasinghe', where: 'Anuradhapura', when: 'yesterday',
  ...
  {
    avatar: 'RD', name: 'Roshan Dissanayake', where: 'Melbourne, AU', when: '3 days ago',
...
function LampOffering() {
  ...
  const [count, setCount] = useState(8432);
  ...
  return (
    <div className="rw-lamp">
      <div className="rw-eyebrow rw-eyebrow--light">Light a Virtual Lamp</div>
      <p className="rw-lamp__lead">Offer a virtual lamp as an act of devotion. Each light represents a moment of mindfulness and merit shared with all beings.</p>
      ...
      <button className={`rw-lamp__btn ${lit ? 'is-lit' : ''}`} onClick={light} aria-label="Light the lamp">
        ...
        <svg viewBox="0 0 64 80" className="rw-lamp__svg" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round">
          {lit && <path d="M32 2 Q26 8 30 14 Q32 11 32 14 Q34 11 34 14 Q38 8 32 2 Z" fill="#f7d94e" stroke="none" className="rw-lamp__flame"/>}
...
function ForumPost({ post }: { post: typeof FORUM_POSTS[0] }) {
    ...
    <div className="rw-page">
      <header className="rw-page__header">
        <div className="rw-container">
          <div className="rw-eyebrow">Community</div>
          <h1 className="rw-page__title">Pilgrim Voices</h1>
          <p className="rw-page__lead">Stories, guidance, and reflections from devotees and visitors. Share your own pilgrimage or read what others have walked through.</p>
        </div>
      </header>
      ...
      <section className="rw-section">
        <div className="rw-container rw-container--prose">
          <div className="rw-forum-head">
            ...
            <button className="rw-btn rw-btn--secondary rw-btn--sm">New Post</button>
          </div>
